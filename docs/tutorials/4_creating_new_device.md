...
In this tutorial, we will add a robot to the Intprim ros framework named Robot1. Robot1 can represent any type of robot (Kuka, Universal Robotics, Willow Garage, etc.) but the finer details will depend on your specific robot and its driver, which are a separate from the interface. Feel free to replace the keyword Robot1 with the name of the robot you are using.
