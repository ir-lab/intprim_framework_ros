...
# Use doxygen comments here or????
# Sphinx/breathe doesn't work with Python 2.
class StatCollector():
    def __init__(self, bip_instance, generated_indices, observed_indices):
        ...
        self.phase_vel_pdf_domain = [-0.02, 0.02]
    ...
    def export(self, bip_instance, export_dir_path, debug_bag_file, response_length):
        ensembles = self.get_projected_ensemble(bip_instance)

        target_phase = None
        if(rospy.get_param("interaction")["single_point_trajectory"]["use"]):
            target_phase = rospy.get_param("interaction")["single_point_trajectory"]["phase"]
        ...
        file_name = export_dir_path + "/stat_collection_" + datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S.xml")
        ...
        spt.text = str(rospy.get_param("interaction")["single_point_trajectory"]["use"])
        spt.set("phase", str(rospy.get_param("interaction")["single_point_trajectory"]["phase"]))
        ...
        if(debug_bag_file != ""):
            bag = rosbag.Bag(debug_bag_file)
            bag_generator = bag.read_messages(topics = [])
            try:
                topic, msg, time = next(bag_generator)
            except Exception:
                print("Failed to open debug bag file.")
                pass
        else:
            bag = None

        # Get the maximum clock time step from the rosbag and get the ratio of the max observed observed timestep.
        # This is the maximum phase observed.
        if(bag is not None):
            max_timestep = bag.get_end_time()
        else:
        ...
        if(bag is not None):
            min_timestep = bag.get_start_time()
        else:
        ...
        factors = ET.SubElement(stats, "factors")

        if(type(bip_instance.basis_model).__name__ == "PLSModel"):
            coefficients = bip_instance.basis_model.transformer.coef_

            start_obs_idx = 0
            end_obs_idx = 0
            for obs_feature_idx in self.observed_indices:
                obs_state_indices = bip_instance.basis_model.basis_model.observed_to_state_indices(obs_feature_idx)

                end_obs_idx += obs_state_indices.shape[0]

                observed_node = ET.SubElement(factors, "observed_factor")
                observed_node.set("name", bip_instance.basis_model.observed_dof_names[obs_feature_idx])

                start_gen_idx = 0
                end_gen_idx = 0
                for gen_feature_idx in self.generated_indices:
                    gen_state_indices = bip_instance.basis_model.basis_model.observed_to_state_indices(gen_feature_idx)

                    end_gen_idx += gen_state_indices.shape[0]

                    # Using start/end because the coefficients factor has indices with respect to each partition of the data.
                    factor_coefs = coefficients[start_obs_idx : end_obs_idx, start_gen_idx : end_gen_idx]
                    ...
                    gen_node = ET.SubElement(observed_node, "generated_factor")
                    gen_node.set("name", bip_instance.basis_model.observed_dof_names[gen_feature_idx])
                    gen_node.set("stride", str(int(factor_coefs.shape[0])))
                    gen_node.text = ",".join(["%.3f" % value for value in factor_coefs.flatten()])

                    start_gen_idx = end_gen_idx

                start_obs_idx = end_obs_idx
